...
# Question Loop
for q in range(5):
    quiz.append(ET.Comment(str(q+1)+' - '+fake.catch_phrase())) # This is a comment
    ...
    ET.SubElement(name, "text").text = str(q+1)+' - '+fake.catch_phrase() # Question name 
    ...
    tmptext = "Question number: "+str(q+1)+"<br>"+fake.catch_phrase() + " ("+MC+"): {1:" + MC + ":~%100%"+fake.word()+"#"+fake.word()+"~"+fake.word()+"#"+fake.word()+"~"+fake.word()+"#"+fake.word()+"} <br>" # Testing MULTICHOICE
...
ET.indent(tree)
tree.write("clozesnum.xml",xml_declaration=True, encoding="utf-8",short_empty_elements=True) # Enabled self-closed tag