...
# Question Loop
for q in range(5):
    quiz.append(ET.Comment("numerical number: "+str(q+1))) # This is a comment

    question = ET.SubElement(quiz, "question", type="numerical") # Question Type

    # Question Name
    name=ET.SubElement(question, "name")
    ET.SubElement(name, "text").text =  "numerical "+str(q+1)+': '+fake.catch_phrase() # Question name 

    # Question Text
    questiontext=ET.SubElement(question, "questiontext", format="html")
    ET.SubElement(questiontext,"text").text = "numerical "+str(q+1)+";<br>"+fake.text() # Question text 
...
ET.indent(tree)
tree.write("numericalsnum.xml",xml_declaration=True, encoding="utf-8",short_empty_elements=True) # Enabled self-closed tag